...
# Configuration here
CALYPTIA_LTS_VERSION=${CALYPTIA_LTS_VERSION:-22.04.1}
...
apt-get update
apt-get -y install docker-ce docker-ce-cli containerd.io docker-compose-plugin git jq httpie python3
systemctl daemon-reload
...
mv -vf /opt/cribl /opt/crible-edge
curl -Lso - "$(curl https://cdn.cribl.io/dl/latest-x64)" | tar zxv -C /opt

cat > /etc/profile.d/20-cribl.sh << EOF
#!/bin/sh
export CRIBL_HOME=/opt/cribl
EOF

# See https://docs.cribl.io/edge/deploy-boot-start to set up a service
if ! /opt/cribl/bin/cribl boot-start disable ; then
    echo "No cribl service to disable"
fi
